// @group BusinessLogic : Port Finder page — lists all open TCP/UDP ports with owning process names
...
export default function PortFinderPage() {
  ...
  async function handleKill(pid: number) {
    ...
    try {
      ...
      if (result.success) {
        // Small delay so the OS has time to clean up, then refresh
  ...
  // @group BusinessLogic > Filtering : Apply search + protocol + state filters
  ...
  // @group Utilities > Styles : Whether a PID can be killed (non-null and > 0)
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 11, color: 'var(--color-muted-foreground)', marginTop: 2 }}>
            {loading ? 'Loading…' : error ? error : `${filtered.length} of ${ports.length} entries · updated ${lastLoad}`}
          </div>
        </div>
              ...
              {filtered.map((entry, idx) => {
                ...
                return (
                  <tr
                    ...
                    style={{ background: hoveredRow === idx ? 'var(--color-accent)' : 'transparent' }}
                  >
                    {/* Port */}
                    <Td mono>
                    ...
                    {/* @group BusinessLogic > Kill : Actions cell — kill button or inline confirm */}
                    <td style={{
                      padding: '5px 12px',
                      borderBottom: '1px solid var(--color-border)',
                      whiteSpace: 'nowrap',
                    }}>
                      {/* Error flash for this row */}
                      {hasError && (
                        <span style={{
                          fontSize: 10, color: 'var(--color-destructive)',
                      ...
                      {/* Only killable rows (pid > 0) get the action UI */}
                      {!hasError && killable && (
                        confirming ? (
                          // @group BusinessLogic > Kill > Confirm : Inline confirm — "Kill process?"
                          ...
                          // @group BusinessLogic > Kill > Button : Kill trigger button
                          <button
                            ...
                            onMouseLeave={e => {
                              e.currentTarget.style.background = 'color-mix(in srgb, var(--color-destructive) 12%, transparent)'
                      ...
                      {/* No action available for PID 0 / no owner */}
                      {!hasError && !killable && (
